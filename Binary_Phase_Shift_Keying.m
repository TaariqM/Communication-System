...
bit_errs_est = bpsk(Nf, SNR, Eb, Na);
bit_errs_est_2 = bpsk(Nf2, SNR, Eb, Na);
for i = 1:length(SNR)
...
title('Bit Error Rate vs. SNR for BPSK');
xlabel('Eb/No (dB)');
ylabel('BER');
grid on
hold on
...
function [BER_est] = bpsk(N,SNR,E,Na)
